...
そこで線形代数を根本的に理解しておきましょう。


線形代数で理解してほしいのは以下の5つです。
...
正方形は直線でできています。正方形の高さと面積の関係は線形代数で表せますか？いえ、表せません。
正方形の高さと面積の関係はこんな感じ。

二次関数
図2

正方形の高さと面積は二次関数です。これは直線ではないから線形代数では扱えないのです。
...
チョコレートパフェはアイスクリーム、チョコシロップ、コーンフレーク、生クリームでできています。
それぞれの重量と、全体にしめる
糖分、脂肪分、水分、カロリー、体積
の関係式を表すと以下のようになります。
...
このようにチョコレートパフェの材料の比率と、糖分、脂肪分、水分、カロリー、体積の関係式は、完全に線形の式で表されます。

では何が線形なのでしょう。たとえばチョコレートパフェにチョコレートを追加しながら糖分の変化を見るとこうなります。
...
これは、X のどの要素とYのどの要素の関係をグラフにしても直線になるということを意味します。
...
たとえば、yの方がわかっていて x を逆算したいときは

x = A^(-1) y - b

で計算できます。また y と x がわかっていてA, bを求めたい場合、
十分な数のx, y がわかっていれば求められます。
...
じゃぁあまり線形代数が使える対象はないのだろうか。しかし線形代数が意外に役立つ理由があります。

なにかを少しだけ変化させた場合、そしてその統計的な効果は

ある範囲で線形とみなせる場合が多い。

たとえば、カットケーキの大きさと、ケーキが売れるか売れないか。これはもちろん非線形であるが、カットケーキを現在の大きさからプラスマイナス10%増減した時に、カットケーキが売れる確率がどう変化するか、これはわりと線形になる。

ですからすでにあるていどうまくいっている物事について、さらにそれをベストな状態に調整していくときには線形代数は大変役立つ、というわけです。


## 2. 5 なにがうれしいか
ではこのような関係式を使って計算を行うと何が嬉しいのでしょうか,,これはひとえに計算機というアルゴリズムが線形代数に向いているということに尽きます
計算機では同様な問題でも老けるわけではありません例えば洗える微分方程式の解を求める効率の良い方法というは存在しません。しかし線形代数の問題の答えは昔から非常に効率の良いアルゴリズムが見つかっていて非常に高速に効率よくを止めることができるのです,計算機自体を線形代数の問題を解くように進歩しています。
ベクトル型計算機という呼び名を聞いたことがありませんがこのベクトルは線形代数のベクトルと厳密に同じでありませんが行くとる方プロセッサーは線形代数の問題を解くのに非常に適しています。
また少し高いパソコンには GPU という計算機がついていますがこれもベクトル計算線形代数の計算2を高速に行う能力を持っています。
なのでもし与えられて問題が線形代数の問題にすることができるその問題は非常に効率よく摂ることができるというわけです。

## 3 線形代数の基礎的理解

### 3.1 多次元空間

しかし、もしxとyが正の係数の比例関係であればxが増加すればyが増えるのはあたりまえで数学を使うまでもなく自明です。ではどのような場合に線形代数は威力を発揮するか。それは次元が非常に多い場合です。

ベクトルというものを覚えていると思います。

y = Ax + b

この式の x も y もベクトルであるわけですつまり x もyも例えば100個の変数であり、Aは非常に大きな行列であったりします。

たとえばwordvec というツールはよく使われますが、これは単語をベクトル化します。単語をベクトル化する、というと難しく思えますが、単語の意味一つ一つを一つの次元とするだけです。
...
大きい = [1/3,0,0,1]
小さい = [-1/3,0,0,-1]
とすることで、大きいという言葉には若干重いというニュアンスも含まれると表すことができるのです。このようにして意味に関係がある言葉は関係があるように、関係がない言葉は関係がないものとして数値で表して処理をすうことができます。

このように線形代数では、非常に多くの次元を含むベクトルを扱います。
ですからこうした高次元をイメージしその性質を把握することが重要になります。
...
逆に100次元のベクトルと行列の積も、座標変換の一種と考えられますね。ですから線形代数ではこれを線形変換と呼ぶわけです。またxとyを線形空間と呼ぶわけです。
...
固有値,固有ベクトル
Ax = λx
となるようなベクトルxを固有ベクトル、λを固有値といいます。

はて、これはいったいどんなベクトルなんでしょう。あるベクトルに行列をかけた結果がそのベクトルにλをかけたものになる。つまり同じ方向のベクトルになる、ということですね。

行列が与えられたときどうやってこのようなベクトルを探すか、わかりやすい方法で探してみましょう。
...
Ax = λx
となるような固有ベクトルをn次元なら最大n個見つけることが可能です。
...
主成分分析では、相関係数行列の固有値を分析して、固有値の大きい方から固有ベクトルを選んでいきます。これが何を意味するかというと、相関行列は、相関係数を用いてある変数の他の変数への影響をあらわしたものとなります。変数xと変数yの相関係数があるかぎり、変数xが変化したときに変数yも若干変化する、と期待できますね。
しかしx, y座標のかわりに固有ベクトルの方向にデータが変化した場合、他の次元に影響があるでしょうか。相関行列を固有ベクトルにかけても係数倍にしかならない、ということは相関行列は固有ベクトルにたいしては相関係数がない、ということと同じです。ですから固有ベクトルの方向にデータが変化してもそれと直交する方向にデータが偏ることはないわけです。
