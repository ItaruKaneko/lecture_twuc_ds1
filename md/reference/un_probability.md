- [確率統計の理解](#確率統計の理解)
	...
	- [3 対立仮説、帰無仮説](#3-対立仮説帰無仮説)
	- [4 有意水準、P値](#4-有意水準p値)
	- [5 なぜこういう用語を理解する必要があるのか?](#5-なぜこういう用語を理解する必要があるのか)
	- [6 t 検定](#6-t-検定)
	- [7 σ(シグマ) と95%信頼区間](#7-σシグマ-と95信頼区間)
	- [8 t 分布](#8-t-分布)
# 確率統計の理解
...
それでは サイコロのやコインの表裏が等しい確率で出るということはどうしてわかるのでしょうか。

これは経験によってそう信じているわけです。しかし本当に表裏の確率が等しいとは限らない。これは実際に試してみると、1/2だと思っていたがそうではなかった、ということがあるわけです。
...
さて、このような検証の仕方をするわけですが、この検証の方法の論理をより厳密に表すために、特別な用語を使います。 それを使って同じことを説明していきましょう。

まず、コインのオモテウラの確率が1/2でない可能性があるか検証したいわけです。 このとき、1/2ではないという仮説を対立仮説と呼びます。

そして、これを検証するためには、後でそれがありえないということを示す仮説を立てるわけですが、この場合はコインの表と裏が1/2の確率であるという仮説になります。そして、この仮説は後で否定しますので、無に帰す仮説ということで帰無仮説と呼びます。 そして、帰無仮説のもとで、観測結果やデータが起こりえる確率を求め、それが妥当であるかどうかで、仮説の妥当性を判断するわけです。 例えば、コインが10回表になったら、帰無仮説が正しいとすれば、それが起こる確率は1/1024となりますね。帰無仮説が正しければ観測されたことは非常に小さい確率でしか起こり得ないので、仮説は疑わしい、と結論付けるわけです。 しかし、ここで「仮説が正しい確率」を求めているわけではありません。仮説が正しいとしたときに、観測結果やデータが起こりえる確率を求めているのです。ということにも注意してください。それによって間接的に、仮説の正しさを判断しているわけです。

しかし、ここで「仮説が正しい確率」を求めているわけではありません。仮説が正しいとしたときに、観測結果やデータが起こりえる確率を求めているのだ、ということにも注意してください。それによって間接的に、仮説の正しさを判断しているわけです。

ここでは、確率と対立仮説・帰無仮説の意味とそれらの使い方について説明しました。まず、これをしっかりと理解してください。

## 4 有意水準、P値

さて確率の議論では、対立仮説と帰無仮説を仮定し、帰無仮説、がおこる確率を求めることができる。
ということがわかりました。
それではこの帰無仮説の確率をどう利用すればよいでしょう?

確率的な事象は、でたらめにおきますから、どんなに実験をくりかえしたりデータを集めても、
仮説が100%ただしいとか、100%間違いである、とは主張できない、ということはわかりますね。
...
そこで、さきほど述べたように、コインを投げるという試行に対する確率的な議論は、帰無仮説の検証、という形で定義して、数値化します。

まず、帰無仮説としてコインAとコインBで表が出る確率は同じである、という仮説を帰無仮説とします。そしてそれぞれの試行を前提として、帰無仮説が成立する確率を求めるのです。この時に使うのがt分布とt検定です。実際にやってみましょう。
...
ここで p-value というのが $p$ 値、つまり帰無仮説である「二つのデータが同じ母集団のデータである」という仮説が正しい場合にこの結果が得られる確率で、2.2e-16 ですから 1京分の2.2である。ほとんどありえない。という結論になります。
...
標本個数を $n$、標本平均を $X$、不偏分散を $S$ の場合 $t$ を
...
なぜ正規分布のかわりに $t$ 分布を使うことがあるのかを説明しましょう。
...
たとえば4サンプルのデータを使ってその平均と分散を計算したとします。
...
$$
s^2 = \frac{1}{4}\sum_{i=1}^4 (X_i-\mu)^2
$$

となります。

データが多ければ, s は $\sigma$ に近くなる。

しかし、たまたま4回歩いただけなので、これらは真の平均値や分散とは違うわけです。

そこでステューデントのt分布が使われます。

計算式は複雑ですがR言語にはこの関数は組み込まれています。
Xにデータがある場合、R言語で t検定を行うのは簡単です。
...
自由度が少なければ差は大きくなりますが、
一回限りの推定であれば、そもそも推定の誤差が
大きいからどちらにしてもたいして正確には
推定できません。

誤差が10もあるのにそれが1減ったところで、
たいして重要ではないと感じるでしょうし、
より正確であると実感することもないでしょう。

しかし、少ないサンプルに基づく推定をくりかえす
場合は威力を発揮します。

t分布はギネス社にやとわれたウイリアム・ゴゼットが
見出したものです。

ゴゼットの課題はビールの品質の安定化でした。

そこでは様々な条件がビールの品質に影響するか否かをできるだけ正確に検定する必要があります。

最初ゴゼットは正規分布を試したが、発発タンクは少なかったので、正規の分布とt分布の差は大きかったのです。

そこでより正確なt分布を使うことを考え出したわけです。

この場合は少ないサンプルによる検定を何度も繰り返すから、
僅かな改善であっても、
それをを繰り返すことは、
ビールの品質を向上するために大変威力を発揮するわけです。
